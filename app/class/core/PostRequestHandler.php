...
class PostRequestHandler
{
    ...
    {
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "savereservationdata"){
        ...
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "setfastorderinfodata"){
            ...
            echo 'fastorderinfodataset';

        }
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "removeZeroQtyProductsFromCart"){
            ...
            if(isset($_SESSION['shopcart']) && is_array($_SESSION['shopcart'])){
                ...
                foreach ($shopcart as $key => $cartprod) {
                    ...
                    $reserved_amount=0;//Product::getProductReservedAmount($cartprod['id']);
                    $pw_amount=Product::getProductWarehouseAmount($cartprod['id']);
                    $free_amount=$pw_amount-$reserved_amount;
                    if($shopcart_amount==0){
                        unset($_SESSION['shopcart'][$key]);

                    }
                    if($free_amount>0 && $free_amount<$shopcart_amount){
                        $_SESSION['shopcart'][$key]['qty']=$free_amount;
                    }
                }
            }

        }
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "checkStockAvailability"){
            include_once("app/class/Product.php");
            $itemCount=0;
            $onStock=0;
            $offStock=0;
            ...
            foreach ($shopcart as $key => $cartprod) {
                ...
                $free_amount=$pw_amount-$reserved_amount;
                if($shopcart_amount<=$free_amount && $shopcart_amount!=0){
                    $onStock++;
                }else{
                    $offStock++;
                } 
            ...
            if($offStock==0 &&  $zeroQuantity==0){
                     echo json_encode(array('status'=>'success', 'message'=>"", 'itemcount'=>$itemCount,'onstock'=>$onStock, 'offstock'=>$offStock, 'zeroquantity'=>$zeroQuantity));  
            } else {
                    echo json_encode(array('status'=>'fail', 'message'=>"Pojedini proizvodi nisu više na stanju!", 'itemcount'=>$itemCount, 'onstock'=>$onStock, 'offstock'=>$offStock, 'zeroquantity'=>$zeroQuantity));    
            }
		...
		if(isset($this->requestVal['action']) && $this->requestVal['action'] == "search"){
			...
			$prodata = Category::getCategoryProductDetail(array_slice($proids,0,40), 1, 18,'','ASC','code',false,'','',1);
        ...
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "setorderinfodata"){
            ...
            $_SESSION['order']['customer']['city'] = $_POST['customercity'];
            $_SESSION['order']['customer']['zip'] = $_POST['customerzipcode'];
            $_SESSION['order']['customer']['voucher'] = $_POST['customervoucher'];
        ...
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "setorderofferinfodata"){
            if(!isset($_SESSION['offer'])){
                $_SESSION['offer']=array();
            }
            if(!isset( $_SESSION['offer']['customer'])){
                 $_SESSION['offer']['customer']=array();
            }
            $_SESSION['offer']['customer']['name'] = $_POST['customername'];
            $_SESSION['offer']['customer']['lastname'] = $_POST['customerlastname'];
            $_SESSION['offer']['customer']['email'] = $_POST['customeremail'];
            $_SESSION['offer']['customer']['phone'] = $_POST['customerphone'];
            ...
            if(isset($_SESSION['shoptype']) && $_SESSION['shoptype'] == 'b2c')
            {
                if(isset($_SESSION['shopcart_request']) && count($_SESSION['shopcart_request']) > 0)
                {
                    $b2c_request_data = Shop::createOfferRequestFromSessionB2C();
                    if($b2c_request_data['status'] == 'success'){
                        //echo json_encode($b2c_request_data);
                        $_SESSION['success_notifications'][] = 'Vaš upit je uspešno prosleđen.';
                        //echo json_encode(array('status'=>'success', "message"=>''));
                ...
                if(isset($_SESSION['shopcart_request']) && count($_SESSION['shopcart_request']) > 0)
                {
                    if(Shop::createOfferRequestFromSessionB2B()){
                        $_SESSION['success_notifications'][] = 'Vaš upit je uspešno prosleđen.';
                    }
                }
            }
...
//                Session::SetArrValNext('shopcart',$prod);
        ...
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "addtocartB2B"){
           include_once("app/class/Product.php");
           $productprice=Product::getProductPrice($this->requestVal['id']);
           $unitstep=$this->requestVal['unitstep'];
           if($this->requestVal['unitstep']=='' || $this->requestVal['unitstep']==0 || is_null($this->requestVal['unitstep'])){
                $unitstep=1;
           }
...
//            $haveattr = ProdHelper::HaveAttr($prodid);//napraviti u prod helper
		...
		if(isset($this->requestVal['action']) && $this->requestVal['action'] == "removeFromCompare"){
            if($_POST['prodid']==0 && $_POST['removeall']==1){
               $res = GlobalHelper::removeAllFromCompare(); 
            } else {
        ...
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "create_offer"){
            ...
            if($continue)
            {
                $b2c_data = Shop::createOfferFromSessionB2C();
                if($b2c_data['status'] == 'success'){
                    echo json_encode($b2c_data);
                    $_SESSION['success_notifications'][] = 'Vaša ponuda je uspešno prosleđena.';
                    echo json_encode(array('status'=>'success', "message"=>''));
                }
                if((!isset($_SESSION['shopcart']) || count($_SESSION['shopcart'])==0)){
                    if(isset($_SESSION['order']["comment"])){
        ...
        /*if(isset($this->requestVal['action']) && $this->requestVal['action'] == "create_offerB2B"){
            //IMPORTANT RESET TO DEFAULT CURRENCY
            global $user_conf, $language;
            include_once("app/class/core/GlobalHelper.php");
            GlobalHelper::resetCurrency();
            ...
            $b2b_data = Shop::createOfferFromSessionB2B();
            if($b2b_data['status'] == 'success'){
                echo json_encode($b2b_data);
                $_SESSION['success_notifications'][] = 'Vaša ponuda je uspešno prosleđena.';
                //echo json_encode(array('status'=>'success', "message"=>''));
  ...
  //       }
        if(isset($this->requestVal['action']) && $this->requestVal['action'] == "userregister"){
            echo User::register($_POST['email'], $_POST['password'], $_POST['passwordconf'], $_POST['name'], $_POST['lastname'], $_POST['adresa'], $_POST['mesto'], $_POST['postbr'], $_POST['telefon'],0, $_POST['defaultlang'],$_POST['birthday']);
        }
		...
		if(isset($this->requestVal['action']) && $this->requestVal['action'] == "fastsearch"){
			include_once("app/class/Search.php");
			echo json_encode(Search::fastProductSearch($_POST['string'], $_POST['categoryid']));
        }	
