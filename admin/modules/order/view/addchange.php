    ...
    <section class="invoice">
      ...
      <div class="row no-print">
        <div class="col-xs-12 verticalMargin10">
          <a  target="_blank" class="btn btn-default jq_printOrderB2CButton hide" ><i class="fa fa-print"></i> Štampaj</a>
		  ...
		  -->
              ...
              <?php global $conn; 
                  ...
                  $q = 'SELECT sw.website, TIME_FORMAT(sw.ts, "%H:%i:%s") AS `time` , TIME_FORMAT(d.ts, "%H:%i:%s") AS `doctime`
                          FROM b2c_document AS d
                          LEFT JOIN statistics_welcome AS sw ON d.ip=sw.ip AND DATE_FORMAT(d.documentdate, "%Y-%m-%d")=sw.`date` AND sw.ts<=d.ts
                          WHERE d.id='.$command[2]." AND d.ip!=''";             
                  //echo $q;  
                  $res = mysqli_query($conn, $q);
                  if(mysqli_num_rows($res) > 0){?>
                    <div class="row no-print" style="background-color: #fff598;">
                      <div class="col-xs-12 verticalMargin10">
                        <h2>Neposredna aktivnost poručioca</h2>
                        <table class="table ">
                          <thead>      
                          <tr>
                            <th style="border-bottom: 2px solid #333;">Došao sa linka</th>
                            <th style="border-bottom: 2px solid #333;">Vreme dolaska</th>
                            <th style="border-bottom: 2px solid #333;">Vreme porudžbine</th>
                          </tr>
                          </thead>
                          <tbody class="jq_orderUserPreActivityCont">
                             <?php while($row = mysqli_fetch_assoc($res)){ ?>
                                  <tr>
                                    <td style="border-bottom: 2px solid #333;"><?php echo $row['website'];?></td>
                                    <td style="border-bottom: 2px solid #333;"><?php echo $row['time'];?></td>
                                    <td style="border-bottom: 2px solid #333;"><?php echo $row['doctime'];?></td>
                                  </tr>
                             <?php }?>
                          </tbody>
                        </table>
