...
//error_reporting(E_ALL);
	...
	function get_item(){
		...
		$q = "SELECT d.id as docid, d.status, d.documentdate, d.warehouseid, d.payment,
		                            dia.quantity, dia.orgquantity, dia.attrvalue, dia.id as documentitemattrid, dia.b2c_documentitemid, dia.status as rowstatus, di.rebate , di.price, di.taxvalue, di.productid, p.name, p.code, p.barcode 
		        FROM b2c_document d 
	...
	function add_New_Item_To_Order(){
		...
		if(mysqli_num_rows($re) > 0){
			...
			$query = "INSERT INTO `b2c_documentitem`(`rebate`, `rebatetype`, `b2c_documentid`, `price`, `price2`, `itemvalue`, `productid`, `productname`, `quantity`, `sort`, `taxvalue`, `taxid`, `ts`) VALUES (   
			...
			$q = "UPDATE `b2c_documentitem` SET itemvalue = quantity*price*((100-rebate)*0.01) WHERE id = ".$documentitemid;
			mysqli_query($conn, $q);
			...
			$query = "INSERT INTO `b2c_documentitemattr` ( `b2c_documentitemid`, `quantity`, `orgquantity`, `attrvalue`, `addedby`, `ts`) 
	...
	function accept_order(){
		...
		if(mysqli_query($conn, $q)){
			...
			send_b2c_email($drow['email'], $user_conf['autoemail'][1], 'Prihvaćena porudžbina', $message);
		}
	...
	function decline_order(){
		...
		if(mysqli_query($conn, $q)){
			...
			send_b2c_email($drow['email'], $user_conf['autoemail'][1], 'Odbijena porudžbina', $message);
...
//echo $q;
	...
	function confirm_Payment(){
		...
		$q = "UPDATE b2c_document SET status = 'p' WHERE id = '".mysqli_real_escape_string($conn, $_POST['orderid'])."' ";
		//echo  $q;
		if(mysqli_query($conn, $q)){	
