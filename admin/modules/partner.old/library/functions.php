...
//PARTNER APPLICATIONS ##############################################################################################################
	function get_partner_applications(){
		...
		/*Filtering*/
		$sWhere = " WHERE (p.id IS NULL OR p.id=0) ";
		...
		if($userid>0 && $partnerapplicationid>0){
			...
			if($rowCount==1){
				...
				if($partnerCode!='' && strlen($partnerCode)>0){
					...
					if($partnerId>0){
						$updateUserQuery="UPDATE user SET partnerid=".$partnerId." WHERE id=".$userid;
...
//PARTNER TRANSACTIONS ##############################################################################################################
	function get_partner_transactions(){
		...
		$sQuery = "	SELECT SQL_CALC_FOUND_ROWS bsi.id, bs.statementdate, bs.number, bsi.description, bsi.debit,bsi.credit 
