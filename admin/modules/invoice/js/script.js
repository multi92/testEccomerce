        ...
        "ajax":{
				...
				dataSrc: function(d){
					...
					for(var i = 0; i < d.aaData.length;i++)
					{
						...
						d.aaData[i][10] += '<input type="text" class="max-width100" /><a class="btn btn-primary btn-xs jq_sendDeliveryEmailButton" ordernumber="'+d.aaData[i][1]+'"><span class="glyphicon glyphicon-send"></span></a>';
						}else if(status == "s"){
							d.aaData[i][10] = d.aaData[i][10] + ' <a class="btn btn-primary btn-xs jq_confirmPaymentButton" ordernumber="'+d.aaData[i][1]+'">Plaćeno<span class="glyphicon glyphicon-usd"></span></a>';
						}
					}
					return d.aaData;
				}
		...
		$('#listtable tbody').on('click', '.deleteButton', function () {
			...
			if($(this).parent('td').find('input').val() != "")
			{
				...
				$.ajax({
					method: "POST",
					url : 'modules/'+moduleName+'/library/functions.php',
					data: { action: "sendDeliveryCodeEmail", code: $this.parent('td').find('input').val(), number: $($this).attr('ordernumber') },
			...
			if(confirm('Plaćeno?'))
			{
				...
				$.ajax({
					method: "POST",
					url : 'modules/'+moduleName+'/library/functions.php',
					data: { action: "confirmPaymentButton", number: $($this).attr('ordernumber') },
					success : function(data){
						if(data == 1){
							$($this).remove();
							window.location.reload();
