	...
	if($(".content-wrapper").attr('currentid') != '')
	{
		...
		$.ajax({
			...
			success:function(response){
				...
				$('.jq_ordpartner').html(a['document']['partnername']);	
				$('.jq_ordpartnercode').html(a['document']['partnercode']);	
				$('.jq_ordpartneraddress').html(a['document']['partneraddress']+', '+a['document']['partnerzip']+' '+a['document']['partnercity']);
				$('.jq_ordpartneremail').html(a['document']['partneremail']);
				if(a['document']['partneraddressid']>0){
					$('.jq_ordpartneraddressobjectname').html(a['document']['partneraddressobjectname']);
					$('.jq_ordpartneraddressaddress').html(a['document']['partneraddressaddress']+', '+a['document']['partneradddresszip']+' '+a['document']['partneraddresscity']);	
				} else {
					$('.invoicepartneraddress').addClass('hide');
				}
				...
				$('.jq_orderDeliveryCost').addClass('hide');
				$('.jq_orderTotalPdv').html(round2Fixed(total_value-total_no_pdv));
				$('.jq_orderTotal').html(round2Fixed(total_value));
