<?php
	 ...
	 */
	$aColumns = array(  'id', 'name', 'adddate', 'changedate', 'status' );
	 ...
	 */
	 //var_dump($_POST['search']['value']);
		...
		if ( $_POST['search']['value'] != "" )
		{
			...
			{
				$sWhere .= "b.".$aColumns[$i]." LIKE '%".mysqli_real_escape_string($conn, $_POST['search']['value'] )."%' OR ";
			}
	...
	$sQuery = "	
		SELECT SQL_CALC_FOUND_ROWS b.id, b.name, b.adddate, b.changedate, b.status, b.position, b.sort, CONCAT(u.name,' ',u.surname) as owner
		FROM  banner as b
...
WHERE ul.id = (SELECT max(id) FROM userlogadmin ul2 WHERE ul2.moduleid = '.$_SESSION['moduleid'].' AND ul2.contentid = '.$aRow['id'].' AND ul2.action = "change")';
		...
		if($lastrow['lastchange'] == ""){
			$row[3] = date( 'd-m-Y H:i:s', strtotime($aRow['changedate']));	
		}else{
