<?php
	 ...
	 */
	 //var_dump($_POST['search']['value']);
		$sWhere = "WHERE ae.name != '' ";
		if ( $_POST['search']['value'] != "" )
		{
			$sWhere .= "AND (";
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				$sWhere .= "ae.".$aColumns[$i]." LIKE '%".mysqli_real_escape_string($conn, $_POST['search']['value'] )."%' OR ";
			}
		...
		$sWhereSupplier = "";		
		if($_POST['supplier'] != 0){
			$sWhereSupplier .= " AND ae.supplierid = ".$_POST['supplier'];
		}
	...
	$sQuery = "	
		SELECT SQL_CALC_FOUND_ROWS ae.id, ae.name, ae.attrid, s.name as suppliername
		FROM  attr_external as ae
		LEFT JOIN suppliers s ON ae.supplierid = s.id
	...
	$q = "SELECT * FROM attr ORDER BY name ASC";
	...
	$localattrstring = '<select class="form-control jq_localAttrSelect"><option value="0"> --- izaberite vrednost --- </option>'.implode("", $localattroptions).'</select>';
	...
	{	
		...
		if($aRow['attrid'] != "0"){
			$localattrstring_tmp = str_replace('value="'.$aRow['attrid'].'"', 'value="'.$aRow['attrid'].'" selected="selected"', $localattrstring);
		}
		$row[0] = $i++;
