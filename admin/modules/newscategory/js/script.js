				...
				$.get(handlerpath+'?operation=get_content&id=' + data.selected.join(':'), function (d) {
					if(d) {
						...
						for(var i = 0; i < (d.lang).length; i++){
							...
							$(clone).find('.jq_label_name').html('Naziv (' + d.lang[i].langname+')');
							$(clone).find('.jq_label_description').html('Opis (' + d.lang[i].langname+')');
						...
						$('.categoryNewIcon').val(d.icon);
						//$(".categoryColorCont").val('');
						//alert(d.color);
						if(d.color != null){
							$(".categoryNewColor").val(d.color);
							$(".mainColorCont").find("span").addClass('hide');
						} else {
							$(".mainColorCont").find("span").removeClass('hide');
						}
						if(d.visible == 'v') $(".categoryVisible").prop('checked', true);
							...
							success:function(response){
								...
								if(a[0] == 0){
									for(var i = 0; i < a[1].length; i++)
									{
										...
										checked = false;
										if(a[1][i][2] == 1) checked = true;
										$(clone).find('.selectIsMandatory').prop('checked', checked);
		...
		$(".saveCategory").on("click", function(){
			...
			$.ajax({
				...
				data: ({action: "savecategory",
						...
						name: namedata,
						visible: $(".categoryVisible").is(':checked') ? 'v' : 'n',
						icon : $('.categoryNewIcon').val(),
						color: $('.categoryNewColor').val() 
						}),
				error:function(XMLHttpRequest, textStatus, errorThrown){
				  alert("ERROR");                            
				},
		...
		$(".addCategory").on("click", function(){
			...
			if($(".newCatNameCont").find(".newCatNameHolder[defaultlang='y']").find(".categoryName").val() != '')
			{
				$(".newCatNameCont").find(".newCatNameHolder").each(function(){
					var obj = {'langid': $(this).attr('langid'),
								...
								'name' : $(this).find('.categoryName').val(),
								'description' : $(this).find('.categoryDescription').val(),
								'icon' : $(this).find('.categoryNewIcon').val(),
								'color' : $(this).find('.categoryNewColor').val()};	
					namedata.push(obj);
				});
			}
			else{
			...
			$.ajax({
				...
				data: ({action: "addcategory",
						id: $(".detailCategoryCont").attr("catid"),
						name: namedata,
						visible: $(".categoryVisible").is(':checked') ? 1 : 0
						}),
				error:function(XMLHttpRequest, textStatus, errorThrown){
				  alert("ERROR");                            
				},
