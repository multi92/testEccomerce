<?php
	 ...
	 */
	$aColumns = array(  'id', 'number', 'adddate', 'predmet', 'status', 'position' );
	 ...
	 */
	 //var_dump($_POST['search']['value']);
		...
		if ( $_POST['search']['value'] != "" )
		{
			...
			{
				$sWhere .= "j.".$aColumns[$i]." LIKE '%".mysqli_real_escape_string($conn, $_POST['search']['value'] )."%' OR ";
			}
	...
	$sQuery = "	
		SELECT SQL_CALC_FOUND_ROWS j.id, j.number, j.adddate, j.predmet, j.status, CONCAT(u.name,' ',u.surname) as owner
		FROM javne as j
		LEFT JOIN user u ON j.owner = u.ID
		$sWhere  
		$sOrder
		$sLimit
	";
...
WHERE ul.id = (SELECT max(id) FROM userlogadmin ul2 WHERE ul2.moduleid = '.$_SESSION['moduleid'].' AND ul2.contentid = '.$aRow['id'].' AND ul2.action = "change")';
		...
		if($lastrow['lastchange'] == ""){
			$row[4] = date( 'd-m-Y H:i:s', strtotime($aRow['adddate']));	
		}else{
			$row[4] = date( 'd-m-Y H:i:s', strtotime($lastrow['changedate']));	
		}	
		$row[5] = $aRow['owner'];
