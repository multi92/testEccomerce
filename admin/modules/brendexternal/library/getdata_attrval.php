<?php
	 ...
	 */
	 //var_dump($_POST['search']['value']);
		$sWhere = "WHERE ave.attrid = ".$_POST['id']." "; 
		if ( $_POST['search']['value'] != "" )
		{
			$sWhere .= "AND (";
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				$sWhere .= "ave.".$aColumns[$i]." LIKE '%".mysqli_real_escape_string($conn, $_POST['search']['value'] )."%' OR ";
			}
	...
	$sQuery = "	
		SELECT SQL_CALC_FOUND_ROWS ave.id, ave.value, ae.attrid, ave.attrvalid
		FROM  attrval_external as ave
	...
	$tmpRow = mysqli_fetch_assoc( $rResult );
	mysqli_data_seek($rResult,0);
	
	$localattroptions = array();
	if($tmpRow['attrid'] != '0')
	{
		$q = "SELECT * FROM attrval WHERE attrid = ".$tmpRow['attrid']." ORDER BY value ASC";
	
		$res = mysqli_query($conn, $q);
		if(mysqli_num_rows($res) > 0){
			while($laRow = mysqli_fetch_assoc($res)){
				array_push($localattroptions, '<option value="'.$laRow['id'].'">'.$laRow['value'].'</option>');
			}
		}
	}
	$localattrstring = '<select class="form-control jq_localAttrvalSelect"><option value="0"> --- izaberite vrednost --- </option>'.implode("", $localattroptions).'</select>';
	...
	{	
		...
		if($aRow['attrvalid'] != "0"){
			$localattrstring_tmp = str_replace('value="'.$aRow['attrvalid'].'"', 'value="'.$aRow['attrvalid'].'" selected="selected"', $localattrstring);
		}
		$row[0] = $i++;
		$row[1] = $aRow['value'];
