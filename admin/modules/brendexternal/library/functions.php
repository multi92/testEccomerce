...
error_reporting(E_ALL);
	...
	if (isset($_POST['action']) && $_POST['action'] != "") {
		switch ($_POST['action']) {
			...
			case "updatelocalbrend" : updateLocalBrend(); break;	
			case "addlocalbrend" : addLocalBrend(); break;	
	...
	function updateLocalBrend(){
		global $conn;
		
		$q = "UPDATE `brend_external` SET `brendid` = '".$_POST['localbrendid']."' WHERE `brend_external`.`id` = ".$_POST['id'];
	...
	function addLocalBrend(){
		global $conn;
		
		$q = "SELECT be.name FROM brend_external be
				LEFT JOIN brend b ON be.name = b.name
				WHERE be.id = ".$_POST['id']." AND b.name IS NULL";
		$res = mysqli_query($conn, $q);
		if(mysqli_num_rows($res) == 1){
			$row = mysqli_fetch_assoc($res); 
			
			$q = "INSERT INTO `brend`(`name`, `sort`, `status`) VALUES ('".$row['name']."',0,'v')";
			if(mysqli_query($conn, $q)){
				$lastid = mysqli_insert_id($conn);
				$q = "UPDATE `brend_external` SET `brendid` = '".$lastid."' WHERE `brend_external`.`id` = ".$_POST['id'];
				if(mysqli_query($conn, $q)){
			...
			echo json_encode(array("status"=>"fail", "message"=>"Brend sa ovim nazivom već postoji!"));	
		}		
	}
