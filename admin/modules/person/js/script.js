...
$(document).ready(function(e) {
	...
	$("#example1").DataTable({
				...
				dataSrc: function(d){
					...
					for(var i = 0; i < d.aaData.length;i++)
					{
						if($("body").attr("user") == "admin")
						{
							...
							var sel = '<select class="form-control selectStatus background-'+d.aaData[i][4]+'" id="'+d.aaData[i][99]+'" currentStatus="'+d.aaData[i][5]+'">';
								sel += '<option value="v" '; if(d.aaData[i][4] == "v") sel +=  " selected "; sel += '>Vidljiva</option>';	
								sel += '<option value="h" '; if(d.aaData[i][4] == "h") sel +=  " selected "; sel += '>Sakrivena</option>';	
        ...
        "language": {
				...
				"infoFiltered":   "(filtrirano od _MAX_ stavki)",
				"infoPostFix":    "",
				"thousands":      ",",
