function deleteItem_handler(elem){
	var $this = $(elem);
	var a = confirm("Da li ste sigurni da zelite da obisete brend?");
	if(a)
	{
		$.ajax({
		  method: "POST",
		...
		$.ajax({
			...
			success:function(response){
				...
				for(var i = 0; i < (a.lang).length; i++){
					...
					$(clone).find(".brendNameInput").val(a.lang[i].name);
					
					$(clone).appendTo($(".addChangeLangCont"));	
				}

				$(".brendImageInput").val(a.image);
				$(".brendLinkInput").val(a.link);
				$(".brendLinkTarget").val(a.link_target);
...
function saveAddChange(){
	...
	if(noerror)
	{
		...
		var passdata = {action: "saveaddchange",
					id: id,
					image: $(".brendImageInput").val(),
					link: $(".brendLinkInput").val(),
					link_target: $(".brendLinkTarget").val(),
					values : []
					};
		$(".langGroupCont:not(.langGroupContTemplate)").each(function(){
			passdata['values'].push({
				...
				name : $(this).find('.brendNameInput').val()
			});
		});	
