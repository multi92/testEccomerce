...
//$user_conf MUST BE INCLUDED BEFORE INCLUDING THIS PAGE 
	...
	if($delivery_cost>0){
		$email_msg.="	<tr style='' class='summaryRow nobackground' style=\"border:none; background-color:transparent; \" >";
		$email_msg.="		<td style='text-align:right;  font-size:10px; border:none !important;' colspan='5'>Troškovi dostave: </td>";
	...
	if($voucher_value > 0){
		$email_msg.="	<tr style='' class='summaryRow nobackground' style=\"border:none; background-color:transparent; \" >";
		$email_msg.="		<td style='text-align:right;  font-size:10px; border:none !important;' colspan='5'>Iznos vaučera: </td>";
	...
	$email_msg.="	<tr style='' class='summaryRow nobackground' style=\"border:none; background-color:transparent; \" >";
	$email_msg.="		<td style='text-align:right; font-weight:bold; font-size:12px; color: #249212; border:none !important;' colspan='5'>Ukupno za plaćanje sa PDV-om: </td>";
	...
	if($paymentData=='u'){ 
		$email_msg.='<br><br><p style="width:100%; text-align:center; font-size:16px;"><b>Molimo vas da uplatu od '.number_format($calc_totalValueWithRebateWithVat, 2).' '.$language['moneta'][1].' izvršite na tekući račun:'.$user_conf["payment slip_bank_account"][1].' ,
			Svrha uplate: "'.$user_conf["payment slip_purpose_of_payment"][1].''.$order_number.'", Primalac: '.$user_conf["company"][1].' '.$user_conf["city"][1].'<br><br> '.$user_conf["footnote_delivery_cost"][1].'
		</b></p>';
	} else {
