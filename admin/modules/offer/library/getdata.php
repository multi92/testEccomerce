<?php
	 ...
	 * you want to insert a non-database field (for example a counter or static image)
	 ...
	 kolone za WHERE
	 */
	$aColumns = array('d.id', 'd.number', 'CONCAT(dd.customername," ",dd.customerlastname) AS ordname', 'd.documentdate', 'd.deliverycode', 'u.email', 'd.admitiondocumentdate');
	 ...
	 */
	 //var_dump($_POST['search']['value']);
		...
		{
			$sWhere = "WHERE d.documenttype IN ('B','BQ') AND  (";
		...
		if($sWhere == '') $sWhere = " WHERE d.documenttype IN ('B','BQ') AND (dd.customeremail != '' OR (dd.customeremail = '' AND d.status = 'bp'))  ";
		else{
			$sWhere .= " AND (dd.customeremail != '' OR (dd.customeremail = '' AND d.status = 'bp')) ";	
		}
	...
	$sQuery = "	
		SELECT SQL_CALC_FOUND_ROWS d.*, TIME_FORMAT(TIMEDIFF(d.timerstart + INTERVAL 1 DAY, NOW()), '%Hh:%im') as timeremaining, dd.customeremail, dd.customername, dd.customerlastname, dd.deliverycost
	...
	$sQuery = "
		...
		FROM  b2c_document AS d LEFT JOIN b2c_documentdetail AS dd ON d.id=b2c_documentid WHERE d.documenttype IN ('B','BQ')
	";
...
LEFT JOIN usercoupon uc ON d.usedcouponid = uc.id
LEFT JOIN coupons c ON uc.couponsid = c.id
WHERE di.b2c_documentid = '.$aRow['id'].' AND dia.status = "a"';
		...
		{
			$q = "UPDATE b2c_document SET status = 'f' WHERE id = ".$aRow['id'];
			mysqli_query($conn, $q);
		}
		...
		$row[2] = number_format($valuerow['totalvalue']+$aRow['deliverycost']+(($valuerow['value'] != NULL)? ($valuerow['value']*(-1)):0) , 2).' '.$aRow['documentcurrency'];
