...
$(document).ready(function(e) {
		...
		$('#listtable tbody').on('click', '.deleteButton', function () {
			...
			if(confirm('Plaćeno?'))
			{
				...
				$.ajax({
					method: "POST",
					url : 'modules/'+moduleName+'/library/functions.php',
					data: { action: "confirmPaymentButton", id: $($this).attr('orderid'), number: $($this).attr('ordernumber') },
					success : function(data){
						if(data == 1){
							$($this).remove();
							window.location.reload();
	...
	$('.jq_createOrderButton').on("click", function(){
		if(confirm('Da li ste sigurni?'))
		{
			var gonext = true;
			if($('.jq_quickOrderBuyerNameField').val() == ''){
				$('.jq_quickOrderBuyerNameField').parent().addClass('has-error');
				gonext = false;
			}
			if($('.jq_quickOrderBuyerLastnameField').val() == ''){
				$('.jq_quickOrderBuyerLastnameField').parent().addClass('has-error');
				gonext = false;
			}
			if($('.jq_quickOrderBuyerAddressField').val() == ''){
				$('.jq_quickOrderBuyerAddressField').parent().addClass('has-error');
				gonext = false;
			}
			if($('.jq_quickOrderBuyerCityField').val() == ''){
				$('.jq_quickOrderBuyerCityField').parent().addClass('has-error');
				gonext = false;
			}
			if($('.jq_quickOrderBuyerZipField').val() == ''){
				$('.jq_quickOrderBuyerZipField').parent().addClass('has-error');
				gonext = false;
			}
			if($('.jq_quickOrderBuyerPhoneField').val() == ''){
				$('.jq_quickOrderBuyerPhoneField').parent().addClass('has-error');
				gonext = false;
			}
			if($('.jq_quickOrderBuyerEmailField').val() == ''){
				$('.jq_quickOrderBuyerEmailField').parent().addClass('has-error');
				gonext = false;
			}
