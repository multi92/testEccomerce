<?php
	 ...
	 */
	 //var_dump($_POST['search']['value']);
		...
		{
			$sWhere = "WHERE d.documenttype NOT IN ('B','BQ') AND  (";
		...
		if($sWhere == '') $sWhere = " WHERE d.documenttype NOT IN ('B','BQ') AND (dd.customeremail != '' OR (dd.customeremail = '' AND d.status = 'bp'))  ";
		else{
	...
	$sQuery = "
		...
		FROM  b2c_document AS d LEFT JOIN b2c_documentdetail AS dd ON d.id=b2c_documentid WHERE d.documenttype NOT IN ('B','BQ')
	";
